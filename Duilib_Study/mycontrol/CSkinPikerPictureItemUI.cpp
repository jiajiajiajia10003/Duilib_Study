#include "CSkinPikerPictureItemUI.h"
...
void CSkinPikerPictureItemUI::PaintStatusImage(HDC hDC)
{
	...
	if ((m_uButtonState & UISTATE_PUSHED) != 0 || (m_uButtonState & UISTATE_HOT) != 0) {
		...
		DrawImage(hDC, _T("file='window\\1.png' fade='150'"));

		//计算作者信息文字和背景图片名字文字的显示位置，这里是用了硬编码，请使用者自己修改
		//RECT rcBkName = m_rcItem;
		m_rcBkName = m_rcItem;
		LONG nTextPadding = (m_rcItem.right - m_rcItem.left - CRenderEngine::GetTextSize(hDC, GetManager(), m_BkName.GetData(), m_iFont, m_uTextStyle).cx) / 2;
		m_rcBkName.left += nTextPadding;
		m_rcBkName.right -= nTextPadding;
		m_rcBkName.top += 15;
		m_rcBkName.bottom = m_rcBkName.top + 20;
		m_rcBkName.left += nTextPadding;
		...
		CRenderEngine::DrawText(hDC, m_pManager, m_rcBkName, m_BkName, kBkNameColor, m_iFont, m_uTextStyle);
